...
#
#   v2.1 更新日志:
#   - [IPv6 增强] 全面添加 ip6tables 支持，与 iptables 同步管理。
#   - [新功能] 增加端口转发管理模块 (NAT/PREROUTING)。
# =================================================================
...
change_ssh_port() {
    ...
    if ! [[ "$new_port" =~ ^[0-9]+$ ]] || [ "$new_port" -lt 1 ] || [ "$new_port" -gt 65535 ]; then
        echo -e "${RED}错误：无效的端口号。${NC}"; return 1
    fi
    ...
    if [ $? -ne 0 ]; then
        ...
        mv "$backup_file" "$SSHD_CONFIG"; return 1
    fi
...
user_management() {
    while true; do
        echo -e "\n===== 用户管理 ====="; echo "1. 新增用户"; echo "2. 修改用户密码"; echo "3. 删除用户"; echo "4. 列出所有用户"; echo "0. 返回主菜单"; echo "===================="
        read -p "请选择操作: " user_choice
        case $user_choice in
            1) add_user ;; 2) change_user_password ;; 3) delete_user ;; 4) list_users ;; 0) break ;; *) echo -e "${RED}无效选择${NC}" ;;
        esac
    done
}
add_user() {
    read -p "请输入新用户名: " username
    if ! [[ "$username" =~ ^[a-z_][a-z0-9_-]*$ ]]; then echo -e "${RED}错误：用户名格式不合法。${NC}"; return 1; fi
    if id "$username" &>/dev/null; then echo -e "${RED}错误：用户 $username 已存在${NC}"; return 1; fi
    useradd -m -s /bin/bash "$username"; echo "请为用户 $username 设置密码:"; passwd "$username"
    if confirm "是否将用户 $username 添加到 sudo/wheel 组？"; then
        if getent group sudo >/dev/null; then usermod -aG sudo "$username"; echo -e "${GREEN}用户已添加到 sudo 组${NC}";
        elif getent group wheel >/dev/null; then usermod -aG wheel "$username"; echo -e "${GREEN}用户已添加到 wheel 组${NC}";
        else echo -e "${YELLOW}警告：未找到 sudo 或 wheel 组。${NC}"; fi
    fi
    echo -e "${GREEN}用户 $username 创建成功${NC}"
}
change_user_password() {
    read -p "请输入要修改密码的用户名: " username
    if ! id "$username" &>/dev/null; then echo -e "${RED}错误：用户 $username 不存在${NC}"; return 1; fi
    echo "请为用户 $username 设置新密码:"; passwd "$username"
...
delete_user() {
    ...
    if [ "$uid" -lt 1000 ] && [ "$uid" -ne 0 ]; then echo -e "${RED}错误：出于安全考虑，禁止删除UID低于1000的系统用户。${NC}"; return 1; fi
    if [ "$username" == "root" ]; then echo -e "${RED}错误：禁止删除 root 用户！${NC}"; return 1; fi
    if confirm "${RED}警告：你确定要删除用户 $username 吗？${NC}"; then
        if confirm "是否同时删除用户的主目录 (/home/$username)？"; then userdel -r "$username"; echo -e "${GREEN}用户 $username 及其主目录已删除${NC}";
        else userdel "$username"; echo -e "${GREEN}用户 $username 已删除（主目录保留）${NC}"; fi
    else echo "操作已取消。"; fi
}
list_users() {
...
detect_firewall() {
    if command -v firewall-cmd &> /dev/null && systemctl is-active --quiet firewalld; then echo "firewalld"
    elif command -v iptables &> /dev/null; then
        if systemctl is-active --quiet iptables || systemctl is-active --quiet netfilter-persistent; then echo "iptables"
        else echo "iptables (inactive)"; fi
    else echo "none"; fi
}

# [IPv6 增强] 初始化iptables时，同时配置ip6tables
enable_iptables() {
    ...
    ip6tables -A INPUT -p ipv6-icmp -j ACCEPT # IPv6的ICMP非常重要，通常需要允许
...
# [IPv6 增强] 开关端口时，同时操作ip6tables
configure_ports() {
    # ... (函数前面部分逻辑不变) ...
    local firewall; firewall=$(detect_firewall)
    if [[ "$firewall" == "none" || "$firewall" == "iptables (inactive)" ]]; then
        echo -e "${RED}错误：防火墙服务未激活。${NC}"; return 1
    fi
    echo "===== 配置防火墙端口 (当前使用: $firewall) ====="; echo "1. 开放端口"; echo "2. 关闭端口"
    read -p "请选择操作: " action
    read -p "请输入端口号: " port
    ...
    if [[ "$firewall" == "iptables" ]]; then
        ...
        save_iptables_rules
        echo "Iptables 规则已保存。"
    elif [[ "$firewall" == "firewalld" ]]; then # firewalld逻辑无变化
        # ... firewalld logic ...
        echo "firewalld 操作暂未实现"
    fi
}

# [IPv6 增强] 保存规则时，同时保存ip6tables规则
save_iptables_rules() {
...
port_forwarding_menu() {
    if [[ "$(detect_firewall)" != "iptables" ]]; then
        echo -e "${RED}错误：端口转发功能目前仅支持 iptables。${NC}"; return
    fi
